...
(function (q) {
    var r = function (d) {
        var j = function (a, b) {
            var c = this;
            if (!this instanceof j)alert("FixedColumns warning: FixedColumns must be initialised with the 'new' keyword."); else {
                "undefined" == typeof b && (b = {});
                d.fn.dataTable.camelToHungarian && d.fn.dataTable.camelToHungarian(j.defaults, b);
        ...
        j.prototype = {fnUpdate: function () {
            ...
            fnRecalculateHeight: function (a) {
                ...
                if ("function" != typeof this.s.dt.oInstance.fnVersionCheck || !0 !== this.s.dt.oInstance.fnVersionCheck("1.8.0"))alert("FixedColumns " + j.VERSION + " required DataTables 1.8.0 or later. Please upgrade your DataTables installation"); else if ("" === this.s.dt.oScroll.sX)this.s.dt.oInstance.oApi._fnLog(this.s.dt, 1, "FixedColumns is not needed (no x-scrolling in DataTables enabled), so no action will be taken. Use 'FixedHeader' for column fixing when scrolling is not enabled"); else {
                    ...
                    this._fnGridSetup();
                    var c;
                    d(this.dom.scroller).on("mouseover.DTFC",function () {
                        c = "main"
                    ...
                    if (0 < b.s.iLeftColumns)d(b.dom.grid.left.liner).on("mouseover.DTFC",
                        function () {
                            c = "left"
                        }).on("scroll.DTFC",function () {
                    ...
                    if (0 < b.s.iRightColumns)d(b.dom.grid.right.liner).on("mouseover.DTFC",function () {
                        c = "right"
                    }).on("scroll.DTFC",function () {
                        "right" === c && (b.dom.scroller.scrollTop = b.dom.grid.right.liner.scrollTop, 0 < b.s.iLeftColumns &&
                    ...
                    f.on("draw.dt.DTFC",function () {
                        ...
                        d(b.dom.scroller).fn("scroll.DTFC mouseover.DTFC");
                        d(q).off("resize.DTFC");
                        d(b.dom.grid.left.liner).on("scroll.DTFC wheel.DTFC mouseover.DTFC");
                        d(b.dom.grid.left.wrapper).remove();
                        d(b.dom.grid.right.liner).on("scroll.DTFC wheel.DTFC mouseover.DTFC");
                ...
                d.each(this.s.dt.aoColumns, function (g, f) {
                    ...
                    if (e.filter(":visible").length) {
                        var h =
                            e.outerWidth();
                        0 === a.s.aiOuterWidths.length && (h += parseInt(d(a.s.dt.nTable).css("border-left-width"), 10));
                ...
                for (var c = [], g = [], f = [], e = 0, h = a.length; e < h; e++) {
                    var k = [];
                    k.nTr = d(a[e].nTr).clone(!0, !0)[0];
                    for (var i = 0, n = this.s.iTableColumns; i < n; i++)if (-1 !== d.inArray(i, b)) {
                        var l = d.inArray(a[e][i].cell, f);
                        -1 === l ? (l =
                            d(a[e][i].cell).clone(!0, !0)[0], g.push(l), f.push(a[e][i].cell), k.push({cell: l, unique: a[e][i].unique})) : k.push({cell: g[l], unique: a[e][i].unique})
                    }
                    c.push(k)
                }
                return c
            }, _fnClone: function (a, b, c, g) {
                var f = this, e, h, k, i, n, l, j, m, o;
                if (g) {
                    ...
                    for (h = m.length; e < h; e++) {
                        k = 0;
                        for (i = m[e].length; k < i; k++)o[e][k].cell.className = m[e][k].cell.className, d("span.DataTables_sort_icon", o[e][k].cell).each(function () {
                            this.className = d("span.DataTables_sort_icon", m[e][k].cell)[0].className
                        })
                    }
                }
                ...
                if (0 < this.s.dt.aiDisplay.length) {
                    h =
                        d(">thead>tr", a.body)[0];
                    for (j = 0; j < c.length; j++)n = c[j], l = d(this.s.dt.aoColumns[n].nTh).clone(!0)[0], l.innerHTML = "", i = l.style, i.paddingTop = "0", i.paddingBottom = "0", i.borderTopWidth = "0", i.borderBottomWidth = "0", i.height = 0, i.width = f.s.aiInnerWidths[n] + "px", h.appendChild(l);
                ...
                if (null !== this.s.dt.nTFoot) {
                    if (g) {
                        ...
                        for (h = m.length; e < h; e++) {
                            k = 0;
                            for (i = m[e].length; k < i; k++)b[e][k].cell.className = m[e][k].cell.className
                        }
                    }
